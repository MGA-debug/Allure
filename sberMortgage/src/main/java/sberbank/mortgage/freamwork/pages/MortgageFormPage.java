...
public class MortgageFormPage extends BasePage {
    ...
    @Step("Заполняем '{nameCondition}' суммой '{sum}'")
    public MortgageFormPage selectCondition(String nameCondition, String sum) {
        WebElement conditionAndSum;

        for (WebElement element : condition) {
            if (element.getAttribute("data-label").toLowerCase().contains(nameCondition)) {
    ...
    @Step("Дополнительные услуни снижающие ставку '{nameAdditionalServices}' согласие '{consent}'")
    ...
    @Step("Проверка на соответствие актуальных/ожидаемых значений")
